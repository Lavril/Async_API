...
Выполните миграции и иницализацию ролей
```bash
make init
make migrate
make roles-init
...
В боевом режиме схема БД управляется через **Alembic**, а не через `Base.metadata.create_all`.

Базовая схема уже настроена в `alembic/env.py` (использует `Base` из `db.postgres` и параметры подключения из `core.config`).

### Основные команды

Из каталога `auth-service`:
...
В `auth-service/makefile` добавлены команды:

```bash
# создать миграцию "init" (autogenerate)
make init
...
Команды выполняются внутри контейнера `fastapi` через `docker compose exec`, поэтому перед этим сервисы должны быть запущены:
...
- в прод‑окружении схема **не создаётся автоматически** — перед запуском приложения нужно прогнать миграции;
- в режиме `debug=True` остаётся возможность авто‑создания/очистки таблиц для локальной разработки (`create_database` / `purge_database`).
...
3. Инициализация ролей и вывод списка ролей
